...
def mnist_shallow():
    mnist_shallow = np.load('mnist_shallow.npy')
    mnist_shallow_sin = np.load('mnist_shallow_sin.npy')
    mnist_shallow_cos = np.load('mnist_shallow_cos.npy')
    ...
    plt.title("Error rate vs epoch for shallow network on MNIST")
    
    plt.savefig('mnist_convergence_shallow.svg')
